    ...
    return (
        <span className="inline-flex items-center gap-0.5" aria-label={`${note} sur ${NOTE_MAX}`}>
            {Array.from({ length: NOTE_MAX }, (_, i) => (
                <Star
                    key={i}
                    className={cn(size, i < note ? "fill-amber-400 text-amber-400" : "text-gray-200")}
                />
            ))}
        </span>
    )
            ...
            <CardHeader className="bg-gray-50/50 border-b border-gray-100 py-4">
                <div className="flex flex-wrap items-center justify-between gap-3">
                    <div className="flex items-center gap-2">
                        <CardTitle className="text-lg font-medium text-gray-700">Satisfaction</CardTitle>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-3">
                    <div className="rounded-xl border border-gray-100 bg-white p-4">
                        ...
                        <div className="flex items-baseline gap-1">
                            ...
                            <span className="text-sm text-gray-400">/ {NOTE_MAX}</span>
                        </div>
                    </div>
                    <div className="rounded-xl border border-gray-100 bg-white p-4">
                        <div className="flex items-center gap-2 text-xs text-gray-500 mb-1">
                    ...
                    <div className="rounded-xl border border-gray-100 bg-white p-4">
                        <div className="text-xs text-gray-500 mb-1">Filtrer</div>
            ...
            <CardContent className="p-6 bg-gray-50/30 min-h-[400px]">
                {loading ? (
                    ...
                    <div className="text-center py-12">
                        <SmilePlus className="w-12 h-12 text-gray-300 mx-auto mb-2" />
                        <p className="text-gray-500">Aucune réponse de satisfaction pour le moment</p>
                    </div>
                ) : (
                    <div className="space-y-3">
                        {satisfactions.map((s) => (
                            <Card key={s.id} className="border-gray-100 hover:shadow-md transition-shadow">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-gray-500 mb-2">
                                                ...
                                                {s.formation && (
                                                    <span className="flex items-center gap-1">
                                                        ...
                                                        <span className="text-gray-400">· {formatDate(s.formation.date_debut)}</span>
                                                    </span>
                                                )}
                                                <span className="text-gray-400">Répondu le {formatDate(s.created_at)}</span>
                                            </div>

                                            {/* Critères détaillés notés */}
                                            {CRITERES_SATISFACTION.some((c) => s[c.key] != null) && (
                                                <div className="flex flex-wrap gap-x-5 gap-y-1.5 mt-2 pt-2 border-t border-gray-100">
                                            ...
                                            {(s.points_forts || s.axes_amelioration || s.commentaire) && (
                                                <div className="mt-3 space-y-2 text-sm">
                                                    ...
                                                    {s.commentaire && (
                                                        <p className="text-gray-600 italic">{s.commentaire}</p>
                                                    )}
                                                </div>
                                            )}
                                        </div>
